package com.xiaoqiang.MyFrame.photo.selector;

import java.util.ArrayList;
import java.util.List;

import com.xiaoqiang.MyFrame.PublishCircleActivity;
import com.xiaoqiang.MyFrame.R;
...
public class PhotoActivity extends Activity {
		...
		photo_bt_del.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (mListViews.size() == 1) {
					...
					String newStr = mDrr.get(mCount).substring(mDrr.get(mCount).lastIndexOf("/") + 1,
							mDrr.get(mCount).lastIndexOf("."));
					mBmp.remove(mCount);
					mDrr.remove(mCount);
					mDel.add(newStr);
					mMax--;
	...
	mCount=arg0;}public void onPageScrolled(int arg0,float arg1,int arg2){// 滑动中
	}
